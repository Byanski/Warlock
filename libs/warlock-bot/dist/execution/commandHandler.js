...
class CommandHandler {
    ...
    async handleMessage(messageContent, callbacks, poller) {
        ...
        if ((gameName === 'start' || gameName === 'stop' || gameName === 'restart') && apiCommand === 'all') {
            ...
            if (!callbacks)
                return `❌ Command not supported in this context.`;
            const waitForStatus = async (targetStatus, pendingServices) => {
            ...
            if (action === 'restart') {
                ...
                for (const svc of allServices) {
                    ...
                    try {
                        await this.client.controlService(svc.guid, svc.host, svc.service, 'stop');
                    }
                    catch (e) { }
                }
                ...
                for (const svc of allServices) {
                    ...
                    try {
                        await this.client.controlService(svc.guid, svc.host, svc.service, 'start');
                    }
                    catch (e) { }
                }
                ...
                for (const svc of allServices)
                    poller.setOverrideStatus(svc.name, null);
                setTimeout(async () => {
                    try {
                        if (msgId)
                ...
                if (isPalworld && palworldCommands.includes(apiCommand)) {
                    if (callbacks) {
                        ...
                        try {
                            ...
                            const adminPasswordConfig = configs.configs?.find((c) => c.option === 'Admin Password');
                            const adminPassword = adminPasswordConfig?.value || '';
                            const restPortConfig = configs.configs?.find((c) => c.option === 'REST API Port');
                            ...
                            const useHttpsConfig = configs.configs?.find((c) => c.option === 'Use HTTPS' || c.option === 'REST API HTTPS');
                            ...
                            switch (apiCommand) {
                                case 'announce':
                                    result = await pwClient.announce(args);
                                    break;
                                case 'kick':
                                    result = await pwClient.kick(args.split(' ')[0], args.split(' ').slice(1).join(' '));
                                    break;
                                case 'ban':
                                    result = await pwClient.ban(args.split(' ')[0], args.split(' ').slice(1).join(' '));
                                    break;
                                case 'unban':
                                    result = await pwClient.unban(args.split(' ')[0]);
                                    break;
                                case 'save':
                                    await pwClient.save();
                                    ...
                                    break;
                                case 'shutdown':
                                    result = await pwClient.shutdown(60, args);
                                    break;
                                case 'force_stop':
                                    result = await pwClient.forceStop();
                                    break;
                                case 'players':
                                    result = await pwClient.getPlayers();
                                    break;
                                case 'info':
                                    result = await pwClient.info();
                                    break;
                                case 'metrics':
                                case 'serverfps':
                                    ...
                                    break;
                                case 'settings':
                                    result = await pwClient.settings();
                                    break;
                            }
                            if (msgId) {
                ...
                if (isMinecraft || isArk || isZomboid || isValheim) {
                    if (callbacks) {
                        ...
                        try {
                            ...
                            const configs = await this.client.getServiceConfigs(guid, host, service);
                            const rconPasswordConfig = configs.configs?.find((c) => c.option === 'RCON Password');
                            const rconPassword = rconPasswordConfig?.value || '';
                            const rconPortConfig = configs.configs?.find((c) => c.option === 'RCON Port');
                ...
                if (isVein) {
                    if (callbacks) {
                        ...
                        try {
                            ...
                            const configs = await this.client.getServiceConfigs(guid, host, service);
                            const httpPortConfig = configs.configs?.find((c) => c.option === 'HTTP Port' || c.option === 'HTTPPort' || c.option === 'Port');
